...
var MARKETS = [
  { id: "del", city: "Delhi-NCR", region: "India", lon: 77.1, lat: 28.6, marketSize: 48e4, entryCost: 55, demandBonus: 0.096 },
  { id: "bom", city: "Mumbai-Pune", region: "India", lon: 72.88, lat: 19.08, marketSize: 52e4, entryCost: 60, demandBonus: 0.104 },
  { id: "blr", city: "Bengaluru-Chennai", region: "India", lon: 77.59, lat: 12.97, marketSize: 4e5, entryCost: 50, demandBonus: 0.08 },
  { id: "ccu", city: "Kolkata / Tier-2", region: "India", lon: 88.36, lat: 22.57, marketSize: 22e4, entryCost: 30, demandBonus: 0.044 },
  { id: "dxb", city: "Singapore / Dubai", region: "Global Export", lon: 55.27, lat: 25.2, marketSize: 15e4, entryCost: 80, demandBonus: 0.03 },
  { id: "sha", city: "Shanghai", region: "Asia-Pacific", lon: 121.47, lat: 31.23, marketSize: 42e4, entryCost: 65, demandBonus: 0.084 },
  { id: "lon", city: "London", region: "Europe", lon: -0.13, lat: 51.51, marketSize: 39e4, entryCost: 70, demandBonus: 0.078 },
  { id: "sfo", city: "San Francisco", region: "North America", lon: -122.42, lat: 37.77, marketSize: 18e4, entryCost: 58, demandBonus: 0.036 },
  { id: "gru", city: "S\xE3o Paulo", region: "Emerging Markets", lon: -46.63, lat: -23.55, marketSize: 46e4, entryCost: 45, demandBonus: 0.092 },
  { id: "nbo", city: "Nairobi", region: "Emerging Markets", lon: 36.82, lat: -1.29, marketSize: 85e3, entryCost: 22, demandBonus: 0.017 }
];
...
__name(marketById, "marketById");
function centreOpenCost(cityIds) {
  if (!cityIds || cityIds.length === 0) return 0;
  return cityIds.reduce((sum, id) => sum + (marketById(id)?.entryCost ?? CENTRE.open), 0);
}
__name(centreOpenCost, "centreOpenCost");
function cityOccupancy(st, cityId) {
  return st.teams.reduce((n, t) => n + ((t.storeCities || []).includes(cityId) ? 1 : 0), 0);
}
__name(cityOccupancy, "cityOccupancy");
function marketBonusOf(st, t) {
...
__name(marketBonusOf, "marketBonusOf");
...
function reachOf(t, salesMult, marketBonus) {
...
function simulateQuarter(st) {
  ...
  for (const t of st.teams) {
    t._hrM = hrMults(st, t);
    t._reach = reachOf(t, t._hrM.sales, marketBonusOf(st, t));
  }
  for (const t of st.teams) {
...
function calculateDemand(state, universeId, policies = [], chargingBoost = {}) {
...
function processQuarterState(state, universeId = "pending", policies = [], batteryDecisions = [], chargingBoost = {}) {
  const quarter = state.quarter;
...
async function runQuarterWorkflow(db, universeId) {
  ...
  const dueLicenses = await db.prepare("SELECT * FROM rd_license_offers WHERE game_id = ? AND status = 'accepted' AND execute_quarter <= ?").bind(universeId, currentQuarter).all();
  const licenseWrites = [];
  for (const offer of dueLicenses.results || []) {
    const buyer = state.teams.find((candidate) => String(candidate.i) === String(offer.buyer_team_id));
    const seller = state.teams.find((candidate) => String(candidate.i) === String(offer.seller_team_id));
  ...
  const chargingRows = state.teams.length ? await db.prepare(`SELECT * FROM charging_network WHERE quarter <= ? AND team_id IN (${state.teams.map(() => "?").join(",")})`).bind(currentQuarter, ...state.teams.map((team) => String(team.i))).all() : { results: [] };
  const chargingBoost = {};
  for (const row of chargingRows.results || []) {
    const team = state.teams.find((candidate) => String(candidate.i) === String(row.team_id));
    if (!team) continue;
...
function briefingRs(value) {
  return `Rs. ${Number(value || 0).toFixed(1)} L`;
}
__name(briefingRs, "briefingRs");
function briefingPct(current, prior) {
  const cur = Number(current || 0);
  const pr = Number(prior || 0);
  if (pr === 0) return cur === 0 ? 0 : 100;
  return (cur - pr) / Math.abs(pr) * 100;
}
__name(briefingPct, "briefingPct");
function describeTeamDecision(decision, submittedBy) {
  ...
  const bits = [];
  if (decision?.ad !== void 0) bits.push(`advertising spend of ${briefingRs(decision.ad)}`);
  ...
  if (decision?.price !== void 0) bits.push(`pricing set to Rs. ${Number(decision.price).toLocaleString("en-IN")}`);
...
__name(describeTeamDecision, "describeTeamDecision");
function buildExecutiveBriefingSections(sourceData) {
  ...
  let performance2;
  if (!current) {
    performance2 = `No settled results are available yet for Q${quarter}. Performance commentary will populate once the quarter has been processed.`;
  } else {
    const cur = current.rawMetrics || {};
    const curScore = Number(current.overall_score || 0);
    if (!prior) {
      performance2 = `Q${quarter} results: revenue of ${briefingRs(cur.revenue)} on ${Math.round(cur.units || 0).toLocaleString("en-IN")} units, ending cash of ${briefingRs(cur.cash)}, and a Balanced Scorecard total of ${curScore.toFixed(1)}. No prior-quarter data exists yet for comparison.`;
    } else {
  ...
  const decisionsText = thisQuarterDecisions.length ? thisQuarterDecisions.map((row) => describeTeamDecision(row.decision, row.submittedBy)).join(" ") : "No decisions were recorded in team_decisions for this quarter.";
  ...
  if (curMetrics.cash !== void 0 && Number(curMetrics.cash) < 200) {
    flags2.push(`Liquidity risk: cash on hand is ${briefingRs(curMetrics.cash)}, below the Rs. 200 L comfort threshold.`);
  }
  if (curMetrics.bankrupt) {
    flags2.push("Insolvency risk: the team's balance sheet is currently flagged bankrupt.");
  }
  if (curMetrics.profit !== void 0 && Number(curMetrics.profit) < 0) {
    flags2.push(`Profitability risk: the team posted a net loss of ${briefingRs(Math.abs(Number(curMetrics.profit)))} this quarter.`);
  }
  if (curMetrics.stockout) {
    flags2.push(`Stockout risk: demand exceeded supply this quarter (${Math.round(Number(curMetrics.lost || 0))} units of lost sales).`);
  }
  if (curMetrics.reliab !== void 0 && Number(curMetrics.reliab) < 0.5) {
    flags2.push(`Reliability risk: product reliability score is ${(Number(curMetrics.reliab) * 100).toFixed(0)}%, below the 50% threshold.`);
  }
  if (curMetrics.util !== void 0 && Number(curMetrics.util) > 0.95) {
    flags2.push(`Capacity risk: plant utilization is at ${(Number(curMetrics.util) * 100).toFixed(0)}%, leaving little room for demand upside.`);
  }
  if (current && prior && Number(current.overall_score || 0) < Number(prior.overall_score || 0)) {
    flags2.push(`Balanced Scorecard total dropped from ${Number(prior.overall_score).toFixed(1)} to ${Number(current.overall_score).toFixed(1)} quarter over quarter.`);
  }
  for (const row of thisQuarterDecisions) {
    const decision = row.decision || {};
    const price = decision.price;
    const segId = decision.segment_id || decision.segmentId;
    if (price === void 0 || !segId) continue;
    ...
    if (Number(price) > wtpMax * 1.15 || Number(price) < wtpMin * 0.5) {
      flags2.push(`Pricing risk: ${decision.brandName || decision.brandId || "a model"} is priced at Rs. ${Number(price).toLocaleString("en-IN")}, far from ${segment.name}'s willingness-to-pay range (Rs. ${wtpMin.toLocaleString("en-IN")}\u2013${wtpMax.toLocaleString("en-IN")}).`);
    }
  }
  const uncertainties = flags2.length ? flags2.join(" ") : "No rule-based risk flags were triggered from this quarter's data.";
  return { performance: performance2, decisions: decisionsText, nextQuarter, uncertainties };
}
__name(buildExecutiveBriefingSections, "buildExecutiveBriefingSections");
async function onRequest(context2) {
  ...
  try {
    ...
    if (path === "/api/battery-lifecycle" && method === "GET") {
      if (!env2.DB) return new Response(JSON.stringify({ projections: [], decision: null, active: Number(url.searchParams.get("quarter")) >= 5 }), { status: 200, headers: corsHeaders });
      ...
      await env2.DB.exec("CREATE TABLE IF NOT EXISTS battery_lifecycle_decisions (id TEXT PRIMARY KEY, universe_id TEXT NOT NULL, team_i TEXT NOT NULL, quarter INTEGER NOT NULL, disposition TEXT NOT NULL CHECK (disposition IN ('warranty', 'repurpose', 'recycle')), returned_units REAL NOT NULL DEFAULT 0, warranty_reserve REAL NOT NULL DEFAULT 0, cost REAL NOT NULL DEFAULT 0, revenue REAL NOT NULL DEFAULT 0, esg_impact REAL NOT NULL DEFAULT 0, created_at TEXT NOT NULL DEFAULT (datetime('now')), updated_at TEXT NOT NULL DEFAULT (datetime('now')), UNIQUE (universe_id, team_i, quarter))");
      const decisionRow = await env2.DB.prepare("SELECT disposition, quarter FROM battery_lifecycle_decisions WHERE universe_id = ? AND team_i = ? AND quarter = ?").bind(universeId, String(teamId), quarter).first();
    ...
    if (path === "/api/battery-lifecycle" && method === "POST") {
      ...
      await env2.DB.prepare("INSERT INTO battery_lifecycle_decisions (id, universe_id, team_i, quarter, disposition) VALUES (?, ?, ?, ?, ?) ON CONFLICT(universe_id, team_i, quarter) DO UPDATE SET disposition = excluded.disposition, updated_at = datetime('now')").bind(id, universeId, String(teamId), quarter, disposition).run();
    ...
    if (path === "/api/d1/pro-forma-statements" && method === "POST") {
      ...
      await env2.DB.exec("CREATE TABLE IF NOT EXISTS pro_forma_statements (id TEXT PRIMARY KEY, universe_id TEXT NOT NULL, team_i INTEGER NOT NULL, quarter INTEGER NOT NULL, statement_json TEXT NOT NULL, created_at TEXT NOT NULL DEFAULT (datetime('now')), updated_at TEXT NOT NULL DEFAULT (datetime('now')), UNIQUE (universe_id, team_i, quarter))");
...
// ../.wrangler/tmp/pages-UZqymf/functionsRoutes-0.8811424048597412.mjs
var routes = [
  {
    routePath: "/api/:route*",
    mountPath: "/api",
