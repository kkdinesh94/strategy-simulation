...
export default function FastTestReport({ teamId, quarter, region = "Global", budget = 0 }) {
  ...
  const purchase = async () => {
    try {
      const response = await fetch("/api/fast-tests", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ teamId, quarter, region, market_research_budget: budget }) });
      const payload = await response.json();
  ...
  if (!purchased) return <div className="flex flex-wrap items-center justify-between gap-3 rounded-lg bg-[#FAF8F5] p-4 text-xs text-[#5A5C60]"><span><LockKeyhole className="mr-2 inline h-4 w-4" />Enter a market research budget, then purchase the report.</span><button type="button" onClick={purchase} disabled={budget <= 0} className="inline-flex items-center gap-2 rounded-lg bg-[#1F2022] px-3 py-2 font-semibold text-white disabled:opacity-40"><ShoppingCart className="h-4 w-4" />Purchase</button></div>;
