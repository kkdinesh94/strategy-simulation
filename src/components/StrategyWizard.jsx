...
export default function StrategyWizard({ universeId, teamId, quarter, teamName, onNotify }) {
  ...
  const updatePlan = (changes) => setPlan((current) => ({ ...current, ...changes }));
  ...
  useEffect(() => {
    ...
    fetch(`/api/strategy-plans?universeId=${encodeURIComponent(universeId)}&teamId=${encodeURIComponent(teamId)}&quarter=${quarter}`)
      ...
      .then((data) => {
        if (!cancelled && data?.plan) setPlan(normalizePlan(data.plan));
      })
      .catch(() => {})
  ...
  const savePlan = async () => {
    if (missionWords > 200 || priorityTotal !== 100) return;
    setStatus("saving");
    try {
      ...
      setStatus("saved");
      onNotify?.("Strategy plan saved for Quarter " + quarter + ".");
    } catch (error) {
      setStatus("error");
      onNotify?.(error.message || "Strategy plan could not be saved.");
    }
  };
  ...
  const renderStep = () => {
    ...
    if (step === 5) return <Field title="Functional strategy priorities" description="Allocate exactly 100 points across the functions. These points express where leadership attention and resources go."><div className="strategy-budget-total"><span>Allocated</span><strong className={priorityTotal === 100 ? "is-good" : "is-error"}>{priorityTotal}/100</strong></div><div className="strategy-slider-list">{FUNCTIONS.map((name) => <label key={name} className="strategy-slider-row"><span><strong>{name}</strong><output>{plan.priorities[name]} pts</output></span><input type="range" min="0" max="100" value={plan.priorities[name]} onChange={(event) => setPriority(name, event.target.value)} /></label>)}</div></Field>;
  ...
  return <section className="strategy-page">
    ...
    <div className="strategy-panel">{renderStep()}<div className="strategy-footer"><button type="button" className="strategy-secondary" disabled={step === 0} onClick={() => setStep(step - 1)}><ArrowLeft size={16} /> Back</button><div className="strategy-footer-right">{status === "saved" && <span className="strategy-saved"><Check size={15} /> Saved</span>}{step < STEPS.length - 1 ? <button type="button" className="strategy-primary" disabled={step === 0 && missionWords > 200} onClick={() => setStep(step + 1)}>Continue <ArrowRight size={16} /></button> : <button type="button" className="strategy-primary" disabled={missionWords > 200 || priorityTotal !== 100 || status === "saving"} onClick={savePlan}><Save size={16} /> {status === "saving" ? "Saving..." : "Save strategy"}</button>}</div></div></div>
  </section>;
}
