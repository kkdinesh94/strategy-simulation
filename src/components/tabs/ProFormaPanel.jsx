...
export function ProFormaPanel({ team, gameState, universeId, onNotify, compact = false, decisionRevision = 0, onExpand = undefined }) {
  ...
  if (compact) {
    const summaryRows = (items) => Object.entries(items).map(([label, value]) => <div key={label} className="flex justify-between py-1.5 text-xs"><span className="capitalize text-[#5A5C60]">{label.replace(/([A-Z])/g, " $1")}</span><strong className="font-mono text-[#1F2022]">{money(value)}</strong></div>);
    return <section className="space-y-3 rounded-xl border border-[#E5E1D8] bg-white p-4 shadow-sm">
      <div className="flex items-center justify-between gap-2">
        <h3 className="text-sm font-bold text-[#1F2022]">Q{statement.quarter} Pro Forma</h3>
        {onExpand && <button onClick={onExpand} className="flex items-center gap-1 text-[10px] font-semibold text-emerald-700 hover:underline">View full <ArrowRight className="h-3 w-3" /></button>}
      </div>
      <div className={`rounded-lg border-2 p-3 ${statement.endingCash > 0 ? "border-emerald-500 bg-emerald-50" : "border-red-500 bg-red-50"}`}>
        <span className="block text-[10px] font-bold uppercase text-[#5A5C60]">Ending Cash</span>
        <strong className={`font-mono text-lg ${statement.endingCash > 0 ? "text-emerald-800" : "text-red-800"}`}>{money(statement.endingCash)}</strong>
      </div>
      <div className="divide-y divide-[#E5E1D8]">
        {summaryRows({ totalInflows: statement.totalInflows, totalOutflows: statement.totalOutflows, netIncome: statement.netIncome })}
      </div>
      <button
        onClick={runSimulation}
        disabled={saving}
        title="Run Production Simulation"
        aria-label="Run Production Simulation"
        className="flex w-full items-center justify-center rounded-lg bg-[#1F2022] py-2 text-white disabled:opacity-60"
      >
        {saving ? <Save className="h-4 w-4" /> : <Play className="h-4 w-4" />}
      </button>
    </section>;
  }
