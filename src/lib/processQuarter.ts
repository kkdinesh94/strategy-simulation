...
export function calculateDemand(state: GameState, universeId: string): DemandResult[] {
  ...
  for (const region of regions(state)) for (const team of state.teams) {
    ...
    for (const model of team.models) for (const segment of SEGMENTS) {
      ...
      const demandUnits = baseSize * brandJudgment / 100 * priceJudgment / 100 * advertisingImpact * salesProductivity * coverage;
      output.push({
...
export function processQuarterState(state: GameState, universeId = "pending"): { state: GameState; logs: QuarterLog[]; demand: DemandResult[] } {
  const quarter = state.quarter;
  const demand = calculateDemand(state, universeId);
